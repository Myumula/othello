...
public:
    ...
    std::string encode_spot_int(int x, int y) {
        if (board[x][y] == BLACK) return "1";
        if (board[x][y] == WHITE) return "2";
        return "0";
    }
    ...
    std::string encode_output_int(bool fail = false) {
        int i, j;
        std::stringstream ss;
        ss << "Timestep #" << (8 * 8 - 4 - disc_count[EMPTY] + 1) << "\n";
        ss << "O: " << disc_count[BLACK] << "; X: " << disc_count[WHITE] << "\n";
        ...
        for (i = 0; i < SIZE; i++) {
            for (j = 0; j < SIZE - 1; j++) {
                ss << encode_spot_int(i, j) << " ";
            }
            ss << encode_spot_int(i, j) << "\n";
        }
        ss << "+---------------+\n";
        ss << next_valid_spots.size() << std::endl;
        if (next_valid_spots.size() > 0) {
            Point p = next_valid_spots[0];
            ss << p.x << " " << p.y << std::endl;
        }
...
int main(int argc, char** argv) {
    ...
    std::ofstream log_int("gamelog_int.txt");
    ...
    while (!game.done) {
        ...
        if (!game.put_disc(p)) {
            ...
            std::cout << data;
            log << data;
            data = game.encode_output_int(true);
            log_int << data;
            break;
        }
        data = game.encode_output();
